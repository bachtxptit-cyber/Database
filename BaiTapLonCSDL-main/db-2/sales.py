...
def create_invoice():
    ...
    try:
        ...
        diem_su_dung = int(d.get('DiemSuDung') or 0)
        if diem_su_dung < 0:
            raise ValueError('Số điểm sử dụng không hợp lệ')

        if diem_su_dung:
            if not d.get('MaKH'):
                raise ValueError('Cần chọn khách hàng để sử dụng điểm')
            kh = db.execute('SELECT DiemTichLuy FROM KhachHang WHERE MaKH = ?', (d['MaKH'],)).fetchone()
            if not kh:
                raise ValueError('Khách hàng không tồn tại')
            if kh['DiemTichLuy'] < diem_su_dung:
                raise ValueError('Khách hàng không đủ điểm')
            max_points = int(max(0, tong_tien - giam) // POINT_VALUE)
            if diem_su_dung > max_points:
                raise ValueError(f'Chỉ được sử dụng tối đa {max_points} điểm cho đơn này')
            giam += diem_su_dung * POINT_VALUE

        tong_sau_giam = max(0, tong_tien - giam)
        
        db.execute('''INSERT INTO HoaDon (MaHD,MaKH,MaNV,NgayLap,TongTien,TongSauGiam,DiemSuDung)
                      VALUES (?,?,?,?,?,?,?)''',
                   (ma_hd, d.get('MaKH'), d['MaNV'],
                    datetime.now().strftime('%Y-%m-%d %H:%M:%S'),
                    tong_tien, tong_sau_giam, diem_su_dung))
        ...
        if diem_su_dung:
            db.execute('UPDATE KhachHang SET DiemTichLuy = DiemTichLuy - ? WHERE MaKH = ?',
                       (diem_su_dung, d['MaKH']))
            db.execute('''INSERT INTO LichSuDiem (MaHD,MaKH,SoDiem,LoaiThayDoi,GhiChu)
                          VALUES (?,?,?,?,?)''',
                       (ma_hd, d['MaKH'], diem_su_dung, 'Tru', 'Sử dụng điểm mua hàng'))
        
        db.commit()
        db.close()
        return jsonify({'message': 'Tạo hóa đơn thành công', 'MaHD': ma_hd, 
                        'TongTien': tong_tien, 'TongSauGiam': tong_sau_giam, 'DiemSuDung': diem_su_dung}), 201
