...
function renderManagers(rows) {
    document.getElementById('managerTable').innerHTML = buildTable([
        ...
        <div class="flex gap-8">
            <button type="button" class="btn btn-outline btn-sm" onclick="viewManager('${row.MaNV}')">Xem</button>
            <button type="button" class="btn btn-primary btn-sm" onclick="editManager('${row.MaNV}')">Sửa</button>
            <button type="button" class="btn btn-danger btn-sm" onclick="deleteManager('${row.MaNV}')">Xóa</button>
        </div>
    `);
}
...
function openManagerForm(mgr = null) {
    const isEdit = !!mgr;
    openModal(isEdit ? `Sửa quản lý ${mgr.MaNV}` : 'Thêm quản lý mới', `
    <div class="form-group"><label class="form-label">Mã NV</label><input class="form-control" id="m_MaNV" value="${mgr?.MaNV||''}" ${isEdit ? 'readonly' : ''} placeholder="NV010" /></div>
    <div class="form-group"><label class="form-label">Họ tên</label><input class="form-control" id="m_HoTen" value="${mgr?.HoTen||''}" placeholder="Nguyễn Văn A" /></div>
    <div class="form-group"><label class="form-label">Email</label><input class="form-control" id="m_Email" value="${mgr?.Email||''}" placeholder="manager@example.com" /></div>
    <div class="form-group"><label class="form-label">SĐT</label><input class="form-control" id="m_SDT" value="${mgr?.SDT||''}" placeholder="0912345678" /></div>
    <div class="form-row">
        <div class="form-group"><label class="form-label">Ngày bắt đầu quản lý</label><input class="form-control" type="date" id="m_NgayBatDau" value="${mgr?.NgayBatDau||''}" /></div>
        <div class="form-group"><label class="form-label">Loại NV</label><select class="form-control" id="m_LoaiNV"><option value="BienChe" ${mgr?.LoaiNV==='BienChe' ? 'selected' : ''}>Biên chế</option><option value="HopDong" ${mgr?.LoaiNV==='HopDong' ? 'selected' : ''}>Hợp đồng</option></select></div>
    </div>
    <div class="form-row">
        <div class="form-group"><label class="form-label">Ngày vào biên chế</label><input class="form-control" type="date" id="m_NgayVaoBienChe" value="${mgr?.NgayVaoBienChe||''}" /></div>
        <div class="form-group"><label class="form-label">Hạn hợp đồng</label><input class="form-control" type="date" id="m_ThoiHanHD" value="${mgr?.ThoiHanHD||''}" /></div>
    </div>
    <div class="form-group"><label class="form-label">Loại hợp đồng</label><input class="form-control" id="m_LoaiHopDong" value="${mgr?.LoaiHopDong||''}" placeholder="Toàn thời gian" /></div>
    <div class="form-group"><label class="form-label">Mã kho</label><input class="form-control" id="m_MaKho" value="${mgr?.MaKho||''}" placeholder="K001" /></div>
    <div class="modal-footer">
        <button class="btn btn-outline" onclick="closeModal()">Hủy</button>
        <button class="btn btn-primary" onclick="saveManager('${mgr?.MaNV||''}')">Lưu</button>
    </div>
    `, true);
}
...
async function saveManager(id = '') {
    ...
    try {
        if (id) {
            ...
            toast('Cập nhật quản lý thành công', 'success');
        } else {
            await apiPost('/employees/', data);
            toast('Thêm quản lý thành công', 'success');
        }
...
async function deleteManager(id) {
    confirmDelete(`Xóa quản lý ${id}?`, async () => {
        try {
            await apiDelete(`/employees/${id}`);
            toast('Xóa quản lý thành công', 'success');
            loadManagers();
        } catch (e) {
            toast(e.message, 'error');
        }
