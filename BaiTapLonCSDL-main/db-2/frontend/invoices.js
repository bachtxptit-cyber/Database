...
function filterInvoices() {
    ...
    renderInvoiceTable(window._invoices.filter(i =>
        ...
        (i.MaKH||'').toLowerCase().includes(q) ||
        (i.MaNV||'').toLowerCase().includes(q) ||
        (i.TenKH||'').toLowerCase().includes(q) ||
        (i.TenNV||'').toLowerCase().includes(q)
    ));
}
    ...
    try {
        ...
        <div class="grid-2">
            <div>
                ...
                <div class="info-item"><label>Tên khách hàng</label><span>${hd.TenKH || 'Khách lẻ'}</span></div>
                <div class="info-item"><label>Tên nhân viên</label><span>${hd.TenNV}</span></div>
            </div>
            <div>
