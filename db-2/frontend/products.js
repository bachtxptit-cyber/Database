    ...
    try {
        ...
        <div class="page-header">
            ...
            <div class="flex gap-8">
                <button class="btn btn-outline btn-sm" onclick="showCategoryModal()">+ Danh mục</button>
        ...
        <div class="card mb-20">
            <div class="search-bar">
                ...
                <select class="form-control" id="spCategory" style="width:160px" onchange="filterProducts()">
                    ...
                    ${categories.map(c=>`<option value="${c.MaDM}">${c.TenDM}</option>`).join('')}
                </select>
            </div>
        </div>
...
function renderProductTable(products, selectedWarehouse = '') {
    const el = document.getElementById('productList');
    if (!el) return;
    el.innerHTML = buildTable([
        {key:'MaSP', label:'Mã SP', cls:'td-name'},
        {key:'TenSP', label:'Tên sản phẩm', cls:'td-name'},
        {key:'TenDM', label:'Danh mục'},
        {key:'Size', label:'Size'},
        {key:'MauSac', label:'Màu sắc'},
        {key:'GiaBan', label:'Giá bán', fmt: v => `<span class="text-gold font-display">${fmtVND(v)}</span>`},
        {key:'TongTon', label:'Tồn kho', fmt: (_, row) => {
            const stock = (window._stockByProduct[row.MaSP] || []).filter(item => !selectedWarehouse || item.MaKho === selectedWarehouse);
            const total = stock.reduce((sum, item) => sum + Number(item.SoLuong), 0);
            const cls = total < 5 ? 'badge-red' : total < 20 ? 'badge-gold' : 'badge-green';
            return `<span class="badge ${cls}">${total}</span>`;
        }},
        {key:'MaSP', label:'Kho', fmt: (_, row) => {
            const stock = (window._stockByProduct[row.MaSP] || []).filter(item => !selectedWarehouse || item.MaKho === selectedWarehouse);
            if (!stock.length) return '—';
            return stock.map(item => `
                <div class="product-stock-row">
                    ${item.TenKho} (${item.MaKho}): <strong>${item.SoLuong}</strong>
                    <button class="btn btn-danger btn-sm" onclick="deleteStock('${item.MaTK}')">Xóa tồn kho</button>
                </div>
            `).join('');
        }},
    ], products, row => `
...
function showProductForm(p = null) {
    ...
    <div class="form-row-3">
        <div class="form-group">
            ...
            <input class="form-control" id="f_Size" value="${p?.Size||''}" placeholder="M, L, XL, 32..." />
        </div>
        <div class="form-group">
...
async function saveProduct(isEdit) {
    ...
    if (!d.MaSP||!d.TenSP||!d.Size||!d.MauSac||!d.GiaBan) return toast('Vui lòng điền đầy đủ thông tin','error');
    try {
