...
function filterImports() {
    ...
    renderImportTable(window._imports.filter(i =>
        ...
        (i.TenKho||'').toLowerCase().includes(q) ||
        (i.TenNCC||'').toLowerCase().includes(q) ||
        (i.TenQLy||'').toLowerCase().includes(q)
    ));
}
...
function renderImportTable(rows) {
    document.getElementById('importTable').innerHTML = buildTable([
        ...
        {key:'TenKho', label:'Kho'},
        {key:'TenNCC', label:'Nhà cung cấp'},
        {key:'TenQLy', label:'Người duyệt'},
        {key:'NgayNhap', label:'Ngày nhập', fmt: v => fmtDate(v)}
    ], rows, row => `
        <div class="flex gap-8">
            <button class="btn btn-outline btn-sm" onclick="viewImport('${row.MaNhap}')">Chi tiết</button>
            <button class="btn btn-danger btn-sm" onclick="deleteImport('${row.MaNhap}')">Xóa</button>
        </div>
    `);
}
...
async function viewImport(id) {
    try {
        ...
        <div class="info-item"><label>Kho</label><span>${phieu.TenKho}</span></div>
        <div class="info-item"><label>Nhà cung cấp</label><span>${phieu.TenNCC}</span></div>
        <div class="info-item"><label>Người duyệt</label><span>${phieu.TenQLy}</span></div>
...
async function deleteImport(id) {
    confirmDelete(`Xóa phiếu nhập ${id}?`, async () => {
        try {
            await apiDelete(`/imports/${id}`);
...
function openImportForm() {
    ...
    <div class="form-row">
        <div class="form-group"><label class="form-label">Kho</label><select class="form-control" id="import_MaKho"></select></div>
...
function renderImportFormOptions() {
    ...
    warehouseSelect.innerHTML = `<option value="">Chọn kho</option>${window._importWarehouses.map(w => `<option value="${w.MaKho}">${w.TenKho}</option>`).join('')}`;
    supplierSelect.innerHTML = `<option value="">Chọn NCC</option>${window._importSuppliers.map(s => `<option value="${s.MaNCC}">${s.TenNCC}</option>`).join('')}`;
