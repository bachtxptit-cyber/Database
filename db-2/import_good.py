    ...
    try:
        ...
        for item in chi_tiet:
            ...
            if not stock:
                db.rollback()
                db.close()
                return jsonify({'error': 'Tồn kho không hợp lệ, không thể xóa phiếu nhập'}), 400

            new_qty = stock['SoLuong'] - item['SoLuong']
            if new_qty < 0:
                db.rollback()
                db.close()
                return jsonify({'error': 'Không thể xóa phiếu nhập vì tồn kho đã nhỏ hơn số lượng nhập'}), 400
